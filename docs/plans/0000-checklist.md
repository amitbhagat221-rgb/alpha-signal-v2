...
_Numbering: [ADR 0015](../decisions/0015-track-numbering-and-rename.md) (tracks) + [ADR 0016](../decisions/0016-plan-numbering-fresh-start.md) (plans). Last updated: 2026-06-15._
...
**▶ NOW (2026-06-15): Track 3.3c realized-return harness SHIPPED — accumulating.** Both the sizing spine ([portfolio_construction.py](../../portfolio_construction.py) → `portfolio_weights`, cockpit card, daily build, [ADR 0044](../decisions/0044-hrp-over-mean-variance-portfolio.md)) AND the HRP-vs-equal-weight head-to-head ([tools/portfolio_outcomes.py](../../tools/portfolio_outcomes.py) → `portfolio_outcomes`, daily PIPELINE_STEPS after `compute_pick_outcomes`; 47 historical books backfilled). All ADVISORY. **Early read (20d, n=16):** HRP −2.01% vs eqw −1.91% (edge **−0.10%**, win 38%), both < tier-NIFTY −0.78% — noisy, 20d≠HRP's horizon, don't over-read. Next 3 concrete actions:
  ...
  2. ✅ **3.3b-3 within-group orthogonalization shipped 2026-06-15** ([tools/factor_marginal.py](../../tools/factor_marginal.py) `--within-group` + `config.SIGNAL_GROUPS`): residualises each factor vs same-group only (cross-group raw — the §3.3b-3 hybrid). **Findings (evidence-only, no weights changed):** Value is the most collinear group every tier (LARGE ρ=0.74 / MID 0.61 / SMALL 0.47) — `book_to_price` dominates within-group incr (SMALL +13.3@252d), `earnings_yield` the redundant member; Analyst NOT collinear (ρ=0.07) so MID `consensus` weakness is poor standalone signal, not redundancy; SMALL Microstructure/Ownership orthogonal (ρ≈0.05-0.11, weights justified).
...
- **3.3 model upgrade** — **GATED on 3.2 completion** (the ~100-factor library + monthly-anchor depth needed for orthogonalization). Per the plan 0002 roadmap this is the **Week 13-24+ / Month 7+ phase — not started as a formal phase yet.** Current status:
   ...
   - ⏳ **3.3c portfolio construction — spine shipped 2026-06-14** ([portfolio_construction.py](../../portfolio_construction.py) → `portfolio_weights`, [ADR 0044](../decisions/0044-hrp-over-mean-variance-portfolio.md)). **HRP, not Markowitz** (§3.3c MV text superseded — error-maximiser on ~15 names from noisy cov + no trustworthy µ): risk-parity weights × bounded alpha tilt (λ=0.6) under per-stock 12% / per-sector 35% / ₹1cr-ADTV caps, Ledoit-Wolf-shrunk cov, ±0.5 daily-return split-defense, marginal-risk-contrib persisted. 2026-06-14 book = 15 names, ex-ante vol 16.3% (<eq-wt 16.9%), eff-N 11.3. **ADVISORY ONLY** (no capital until `validate_rank_skill` clears, ~2026-07-06). Cockpit `/portfolio` **sized-book view shipped 2026-06-14** ([api.get_sized_book](../../cockpit/api.py) → [portfolio.html](../../cockpit/templates/portfolio.html), reads `portfolio_weights`, shows HRP weight + %-risk + eff-N/caps + **expected-1Y return** (book-weighted analyst PT upside, covered names only — same source as the page's existing Expected Return stat); renders 200, live after next prod restart). **Daily build wired into PIPELINE_STEPS** right after `screener` (non-critical; `portfolio_construction.run`) — runs on the existing 03:30 UTC cron, no separate cron line; `portfolio_weights` now freshness-tracked (asof_date added to `_table_date_range` DATE_COLS, health 82→83 fresh). **3.3c spine complete.** **Realized-return harness shipped 2026-06-15** ([tools/portfolio_outcomes.py](../../tools/portfolio_outcomes.py) → `portfolio_outcomes`; 47 books backfilled, daily after `compute_pick_outcomes`, 35d staleness override mirroring `pick_outcomes`): per asof_date × window, HRP-weighted vs equal-weight-same-names vs tier-blended NIFTY. Early 20d read (n=16) HRP≈eqw (−0.10% edge). **Remaining:** risk-adjusted (NAV/Sharpe) comparison + the §3.3c hard gate (≥1.5% risk-adj over 18-24mo — needs the history now accumulating). · 💤 3.3d Barra-style risk model.

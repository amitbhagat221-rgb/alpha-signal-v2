...
## Next 3
1. ⏳ **Plan 0005 Phase D — backtest depth (88 → 90)** — Phase C now ~80% done in this session (regulatory feed alive + coverage-regression armed + yfinance price fallback 86% → 99.9% universe coverage). Phase D extends PIT from 36mo to 60mo using v1 archive, adds bootstrap CIs on t-stats, rejects KEEP with n<12. ~2 sessions. Highest leverage now: validate the new B/B+ KEEP signals on more data.
2. ⏳ **Budget decision: regulatory classifier restart** — raw `regulatory_events` is flowing (24,025 events, fresh through May 2026) but `regulatory_signals` (classified) still 44d stale. Options: (a) run on the 1904 newest events only (~$5-10 Anthropic), (b) full rebuild on all 24K events (~$100+), (c) remove regulatory factor from production until decided. User call.

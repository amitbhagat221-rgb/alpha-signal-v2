...
**▶ NOW (2026-06-15): Track 3.3c → wire the HRP book into the realized-return harness.** 3.3c sizing spine shipped today ([portfolio_construction.py](../../portfolio_construction.py) → `portfolio_weights`, cockpit `/portfolio` card, daily PIPELINE_STEPS build, [ADR 0044](../decisions/0044-hrp-over-mean-variance-portfolio.md); all ADVISORY). Next 3 concrete actions:
  1. HRP book vs equal-weight Model Portfolio through the realized-return loop ([paper_portfolio.py](../../paper_portfolio.py) / [tools/compute_pick_outcomes.py](../../tools/compute_pick_outcomes.py)) — the §3.3c gate evidence; book history accumulating since 2026-06-15.
  2. 3.3b-3 within-group orthogonalization ([tools/factor_marginal.py](../../tools/factor_marginal.py) has steps 1-2).
  3. `credit_beta` un-bench via jugaad-data `stock_prices`→2018 (free quick-win) — OR `pt_upside` re-verify (gated ~2026-08-01).

...
## Next 3
1. ⏳ **Phase 2.2 db.py wiring (gated on parallel session)** — add `financial_signal` to `daily_snapshots_pit` via `_COLUMN_MIGRATIONS` + BACKTEST_SIGNALS entry. Currently blocked: parallel session has uncommitted db.py edits (DuckDB work). Pick up after they merge.
2. ⏳ **Phase 2.2d — PIT helper + backtest validation** — `tools/reconstruct_pit.py` `pit_financial` helper joins `banking_metrics.period_end ≤ pit_date - 75d` (annual filing lag); backtests financial_signal IC within Financial Services subset (need t-stat ≥ 2.0 per Plan 0001 done gate). Gates the screener routing change.
3. ⏳ **Phase 2.2c — NBFC GNPA fallback** — 80/100 NBFCs missing quarterly NPA. RBI XBRL / SEBI Form A / AR scrape. Required for NBFC scoring to mature beyond 2-component basis.
4. ⏳ **Wire 2 non-colinear bench factors + variant promotion + refresh `nse_index_history`** — all still queued.

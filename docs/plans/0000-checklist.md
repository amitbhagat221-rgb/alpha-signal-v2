...
## Next 3
1. ⏳ **Phase 2.2b-v2 — tier-aware financial_signal** — backtest showed LARGE/MID respond inversely to SMALL on NPA. Two paths: (a) per-tier direction from rolling-IC, (b) split into two distinct signals (`financial_quality` for SMALL, `financial_recovery` for LARGE/MID). Path (b) is cleaner architecturally — names the underlying alpha mechanism. Re-run PIT + backtest; promote whichever path clears t-stat ≥ 2.0 in ≥ 2 tiers.
...
- ⏳ 2.2 Financial sub-model — **source decision flipped 2026-05-29: Screener.in, not Tickertape** ([ADR 0030](../decisions/0030-banking-metrics-screener-first.md)). Probe showed Tickertape carries no banking-specific ratios; Screener.in stock pages have GNPA/NNPA/NII/Interest/Deposits. Scope clarified to **158 Banks+NBFCs** (the 91 AMC+Insurance+Capital-Markets stay on main screener).
   ...
   - ⚠ Phase 2.2d (partial): PIT helper + backtest **DONE 2026-05-29**. 148-date PIT reconstruction, 14.5K rows. **Composite FAILS done gate** (t = -0.75 / -1.30 / -0.34 for LARGE/MID/SMALL). **DIAGNOSTIC SURFACED A REAL FINDING**: components have predictive power but signs flip by tier — LARGE/MID `net_npa_pct` t=+2.39/+4.16 (mean reversion / distressed recovery), SMALL `gross_npa_pct` t=-3.09 (quality compounds). My composite assumed direction='lower' universally, INVERTING the LARGE/MID signal. `screener.py` routing remains correctly gated (no live impact). Next: tier-aware sign recalibration OR split into "quality" (SMALL) + "recovery" (LARGE/MID) sub-signals.

...
_Last updated: 2026-05-31 (Plan 0006 closed end-to-end — Phase D LLM sector dossiers + Phase E `sector_momentum` factor & S/M/L horizon badges; plus corp_actions daily wiring + slug-override registry) · Plans are truth, this is the view. Update via `/handoff`._
...
## Next 3
1. ⏳ **Deploy Phase E horizon badges** — `sudo systemctl restart alpha-cockpit.service`; live `:3000/sectors` shows 0 `sd-horizons` because [cockpit/api.py](../../cockpit/api.py) + [cockpit/templates/sectors.html](../../cockpit/templates/sectors.html) changed after the last restart. Confirm 11 sectors render S/M/L badges.
2. ⏳ **Track 3.1b — NSE F&O OI** (active; unblocks §3.2.2 options cluster) — extend [sources/nselib_pull.py](../../sources/nselib_pull.py) `pull_fii_positioning` to participant/strike OI; design schema + daily/forward cron.
3. ⏳ **sector_momentum medium-horizon breadth trend** — fold a 4-week `breadth_pct` trend into the medium horizon in [signals/sector_momentum.py](../../signals/sector_momentum.py) once `sector_briefs` has ≥4 weeks of history (only 3 days as of 2026-05-31; currently medium = 3m price-RS only).

...
_Last updated: 2026-05-24 (handoff #2 of day — 28-item audit + cadence-aware backtest framework with Newey-West; 3 new KEEP-verdict signals uncovered; analyst-consensus 59.3% provenance leak surfaced) · Plans are truth, this is the view. Update via `/handoff`._
...
## Next 3
1. ⏳ **Fix `ANALYST_CONSENSUS_GROWTH_WITHOUT_ANALYSTS` at source** — sanity check fires CRITICAL at 59.3% (1447/2440 stocks have growth pcts but NULL `total_analysts`/`price_target`). Growth from Tickertape forecast feed; PT/count from yfinance. When yfinance has nothing, `consensus_signal` still fires on growth-only. Either gate growth in [sources/tickertape_analyst.py](../../sources/tickertape_analyst.py) write path on PT/count presence, or update [signals/consensus.py](../../signals/consensus.py) to require at least one analyst-sourced field before emitting.
2. ⏳ **Extend historical_backfill to FII/DII Cash Segment** — different SEBI source than the F&O positioning shipped today. Add `backfill_fii_cash()` paralleling `backfill_fii_fno()` in [sources/historical_backfill.py](../../sources/historical_backfill.py); endpoint `https://www.nseindia.com/api/historical/foreignActivity`. ~1hr code + ~30min pull. Then re-run reconstruct_pit to populate the new column.

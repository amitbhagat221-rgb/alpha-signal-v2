...
Status: implemented
Created: 2026-05-10
Last updated: 2026-05-11 (shipped — see "Final form" at bottom of file)
Owner: Amit Bhagat
Implementation: shipped 2026-05-11 — /sectors deep-dive live with industry-level drill + LLM narratives
Related: 0005-100-factors-and-model.md, 0007-market-share-momentum-factor.md, ADR 0009 (factor-track), ADR 0013 (industry drill), ADR 0014 (LLM competitive landscape)
Source: docs/_archive/Sector Narratives.pdf (IIM Ahmedabad Consult Club Casebook 2020-21)
---

> **Final form (2026-05-11) — what actually shipped vs what the plan proposed:**
>
> **Shipped end-to-end:**
> - `/sectors` rewritten with industry-primary drill (38 industries grouped under 11 sector headers in the heatmap; click a card → industry detail). Decision: ADR 0013.
> - Industry detail page has 4 sub-tabs (Overview / Players / Trends / Our Picks), matching the IIM template (value chain, segments+KPIs, drivers, regulators, cyclicality, India-specific) plus our model's view layered on (composite score, picks, factor means).
> - 37 industry narratives generated by Sonnet 4.6 + web search via [tools/sector_narrative_fetcher.py](../../tools/sector_narrative_fetcher.py). Stored in `sector_metadata`, keyed by industry. 0-hand-curation budget as locked in.
> - **Beyond the original plan:** added `competitive_landscape` (top 6-14 players per industry, listed + private, with `share_basis` and `as_of`) — replaces the previous mcap-only Top Players view that mis-stated reality (IndiGo "100%" when Air India carries 26%). Decision: ADR 0014.
> - Composite score switched from equal-weighted AVG to market-cap-weighted Σ(score × mcap) / Σ(mcap). Decision: folded into ADR 0013.
> - From-scratch industry classifier ([tools/classify_industries.py](../../tools/classify_industries.py)) reclassified all 2,448 stocks, overwriting wrong source GICS sector tags (Eternal/Zomato fix).
>
> **Deferred / out of scope (per the lock-in):**
> - Live factor-health within an industry (gated on F-track factor breadth — plan 0005).
> - Sector→stock visual hierarchy beyond cards (would require D3-style network viz; not worth the build cost for 38 nodes).
> - Time-series view of composite score (gated on a daily snapshot history — currently we only have point-in-time).
>
> **Commits:** `b729bdb` (Alpine fix), `a8bc1ec` (stock_detail design vocabulary), `0b86c12` (industry classifier), `84078d9` (mcap share %), `94bfc91` (from-scratch classifier writes parent sector), `5b6d1cb` (industry drill + mcap-weighted composite), `b3f295a` (competitive landscape), `903de71` (strict ticker match).
>
> **Plan body below preserved as written 2026-05-10 for historical context.**

...
1. **Sector vs industry granularity?** GICS Financials covers Banking + Asset Mgmt + Insurance + NBFCs. Show one industry by default, or surface an industry-picker within each sector? **My take:** start with one (dominant by stock count), add picker if gap shows up.

2. **Market-trends generation — LLM weekly or hand-curated?** LLM is cheaper and stays current; hand-curated is more controlled. **My take:** LLM weekly with caching, hand-edit override stored in same `sector_trends` table (`source` column: `'auto'` or `'manual'`).

3. **Where does this sit in plan-0005's roadmap?** This is technically D17/D18 territory (sector-aware portfolio construction). It uses F-track factor data (per-sector IC needs `pit_ic_by_tier_v2` extension). **My take:** ship as standalone — not blocking F-track or D-track, but a complementary deliverable that uses both.

4. **Tab 2 v1 — descriptive (sector mean of factor) or wait for v2 (per-sector IC)?** Most of the page's value is in panels A-E + the picks/macro/regulatory sub-panels — those don't need IC. **My take:** ship v1 descriptive; promote IC to "live" when the backtest extension is done.
...
- Industry-level peer comparisons within a sector (deferred — needs the granularity decision in Q1 above)

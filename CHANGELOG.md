...
- **Split-adjustment parser at 100% coverage.** [tools/compute_splits.py](tools/compute_splits.py) regex extended to handle the canonical NSE `Face Value Split (Sub-Division) - From Rs X/- Per Share To Rs/Re Y/- Per Share` format and the `Re` (singular Rupee, used when value is 1) marker. Parse rate **104/207 → 207/207**. [tools/apply_splits.py](tools/apply_splits.py) reapplied: `stock_prices.adj_close` now adjusted across **102,315 (sid, date) rows on 188 stocks** (was 58,263 / 101). Apples-to-apples diagnostic across 12 v1 snapshot dates: split adjustment delivers **+0.12 mean Pearson lift** on mom_12m (raw close 0.74 → adj_close 0.86); +0.10 mean Spearman lift (0.76 → 0.87). The earlier 0.67→0.78 figure used a different reconstruction methodology (different lookback window). Remaining v1 gap = dividend adjustment.
- **Smart-beta indices: 13 indices populated** (was 9). Fixed three invalid names in `SMART_BETA_INDICES` ([sources/nselib_pull.py:432](sources/nselib_pull.py#L432)) — the names below match NSE's index master exactly: `NIFTY100 LOW VOLATILITY 30`, `NIFTY ALPHA LOW-VOLATILITY 30`. Added `NIFTY200 QUALITY 30` and `NIFTY100 EQUAL WEIGHT`. All 4 backfilled at 743 rows each (2023-05-08 → 2026-05-06). **Gotcha discovered:** nselib's `cm.index_data` silently caps wide-range queries at ~70 trading days regardless of `from_date`/`to_date`. Always paginate via `_months_back(N)` (monthly chunks). The first backfill attempt with 12-month chunks hit this and only landed 210 rows per index before re-running.
